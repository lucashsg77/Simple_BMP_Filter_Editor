...
void High_pass::kernel_creation(int radius)
{
    ...
    //For the sharpen filter the sum of the kernel elements needs to be 1, what differs this filter from the smoothing one is that the contrast between the central pixel and it's neighbours needs
    //to be enhanced, therefore negative values are attributed to the neighbour pixels.
    for (int i = 0; i < size; i++) {
        if (i == (size) / 2) kernel_1d[i] = row_kernel * col_kernel;
